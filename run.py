...
pred=np.squeeze(ans, axis=-1)
pred=np.where(pred >=0.5,1,0)


start=np.multiply(start, np.asarray(y_mask_iuput)[:,1:text_maxlen+1])
start_index=np.argmax(start, axis=1)

end=np.multiply(end,np.asarray(y_mask_iuput)[:,1:text_maxlen+1])
    ...
    for k in j['qas']:
      if  pred.tolist()[idx]==0:
        ...
        if end_index.tolist()[idx]-start_index.tolist()[idx]>30 or end_index.tolist()[idx]<=start_index.tolist()[idx]:
          ...
          flag=False
          candi_end=np.argsort(end[idx])[-5:][::-1].tolist()
          candi_start=np.argsort(start[idx])[-5:][::-1].tolist()
